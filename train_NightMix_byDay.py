import os.path
from common import get_mix_train_ds, get_val_ds, cosine_scheduler, log_data
import time
...
drop_out = 0.5
initial_lr = 0.005
exclude_days = [5]  # 排除已经训练过的和精度为100的
block_size = 14
...
def main():
    for iCount in range(8):
        ...
        for i in range(3):
            # 路径
            base_root = r"/inventory_by_day/data_{}/"
            train_path = base_root.format(day) + "train_07_Gray_merge_LR/"
            val_path = base_root.format(day) + "val/"
            now_time = datetime.now().strftime("%Y%m%d-%H%M%S-%f")  # 格式化时间字符串

            # 归类文件
            floder_name = "RMA_mobile_train_NightMix_byDay"
            if not os.path.exists("./{}".format(floder_name)):
                os.makedirs("./{}".format(floder_name))

            log_name = "./{}/{}_RMA_NightMix.csv".format(floder_name, now_time)
            best_acc_log = "./{}/best_val_acc.csv".format(floder_name)
            weight_dir = "{}/{}_weights_RMA_NightMix".format(floder_name, now_time)
            log_data(log_name,
                     ["Epoch", "Wall Time", "Train Time", "Train Loss", "Train Accuracy", "Validate Loss",
                      "Validate Accuracy", "Best Validate Accuracy"])

            log_dir = "./{}/boards/{}_RMA_NightMix".format(floder_name, now_time)
            ...
            # 获取数据集
            train_ds = get_mix_train_ds(train_path, im_height, im_width, 0, batch_size)
            ...
            for epoch in range(epochs):
                mask_size = round(epoch / epochs) * block_size
                train_ds = get_mix_train_ds(train_path, im_height, im_width, mask_size, batch_size)
                ...
                if val_accuracy.result() > best_val_acc:
                    ...
                    model.save_weights("./{}/RMA_ecsa.ckpt"
                ...
                if best_val_acc == 1:
                    log_data(best_acc_log, [day, now_time, best_val_acc.numpy()])
