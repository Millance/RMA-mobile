...
def main():
    ...
    # 归类文件
    floder_name = "RMA_mobile_train_Gray"
    if not os.path.exists("./{}".format(floder_name)):
        os.makedirs("./{}".format(floder_name))

    log_name = "./{}/{}_RMA_Gray.csv".format(floder_name, now_time)
    weight_dir = "{}/{}_weights_RMA_Gray".format(floder_name, now_time)
    log_data(log_name,
             ["Epoch", "Wall Time", "Train Time", "Train Loss", "Train Accuracy", "Validate Loss",
              "Validate Accuracy", "Best Validate Accuracy"])

    log_dir = "./{}/boards/{}_RMA_Gray".format(floder_name, now_time)
    ...
    best_acc_log = "./{}/Gray_best.csv".format(floder_name)
